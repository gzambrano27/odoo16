            ...
            if row.batch_payment:
                ...
                if rec_lines:
                    for line in rec_lines:
                        invoice_move_lines = line.invoice_id.line_ids.filtered(
                            lambda r: not r.reconciled and r.account_type in ("liability_payable", "asset_receivable")
                        )
                        payment_move_lines = line.payment_id.move_id.line_ids.filtered(
                            lambda r: not r.reconciled and r.account_type in ("liability_payable", "asset_receivable")
                        )

                        amount_remaining = abs(line.amount_paid)

                        # Ordenar por saldo para priorizar líneas con menor valor primero (opcional)
                        invoice_move_lines = invoice_move_lines.sorted(
                            key=lambda r: (r.date_maturity or r.date, abs(r.amount_residual))
                        )
                        payment_move_lines = payment_move_lines.sorted(key=lambda r: abs(r.amount_residual))

                        for invoice_line in invoice_move_lines:
                            for payment_line in payment_move_lines:
                                if amount_remaining <= 0:
                                    break

                                # Calcular el monto posible a reconciliar entre estas dos líneas
                                ...
                                amount_remaining -= amount_to_reconcile
                                move_lines |= invoice_line + payment_line

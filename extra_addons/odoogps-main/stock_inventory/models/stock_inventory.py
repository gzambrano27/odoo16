...
class Inventory(models.Model):
    ...
    def action_validate(self):
        ...
        self._check_company()
        quants.write(
            {"inventory_quantity_set": False, "last_inventory_date": self.date}
        )
        return True
    ...
    def action_open_inventory_lines(self):
        ...
        if self.product_ids:
            ...
            action["view_id"] = self.env.ref(
                "stock_inventory.stock_inventory_line_tree_no_product_create"
            ).id
            ...
            action["view_id"] = self.env.ref(
                "stock_inventory.stock_inventory_line_tree"
            ).id
    ...
    def action_print(self):
        return self.env.ref("stock_inventory.action_report_inventory").report_action(
            self
        )
    ...
    def _get_quantities(self):
        ...
        locations_ids = [
            loc["id"]
            for loc in self.env["stock.location"].search_read(domain_loc, ["id"])
        ]
    ...
    def _get_inventory_lines_values(self):
        ...
        if self.exhausted:
            vals += self._get_exhausted_inventory_lines_vals(
                {(loc["product_id"], loc["location_id"]) for loc in vals}
            )
        return vals

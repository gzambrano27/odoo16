...
class MailThread(models.AbstractModel):
    ...
    def _notify_get_recipients(self, message, msg_vals, **kwargs):
        ...
        if (
            "notify_followers" in self.env.context
            and not self.env.context.get("notify_followers", False)
            or "force_partners_to_notify" in self.env.context
            and self.env.context.get("force_partners_to_notify")
        ):
            ...
            if self.env.context.get("force_partners_to_notify"):
                pids = self.env.context.get("force_partners_to_notify")

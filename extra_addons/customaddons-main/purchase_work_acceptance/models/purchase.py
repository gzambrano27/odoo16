...
class PurchaseOrder(models.Model):
    ...
    wa_invoice_ids = fields.One2many(
        comodel_name='account.move',
        inverse_name='wa_id',
        compute='_compute_wa_invoice_ids',
        string='Facturas desde RS',
        store=False,
    )
    wa_invoice_count = fields.Integer(
        string='Cantidad de Facturas RS',
        compute='_compute_wa_invoice_ids',
        store=False,
    )

    def _compute_wa_invoice_ids(self):
        for order in self:
            invoices = self.env['account.move'].search([
                ('wa_id', 'in', order.wa_ids.ids)
            ])
            order.wa_invoice_ids = invoices
            order.wa_invoice_count = len(invoices)

    def action_view_wa_invoices(self):
        self.ensure_one()
        action = self.env.ref('account.action_move_in_invoice_type').read()[0]
        action['domain'] = [('id', 'in', self.wa_invoice_ids.ids)]
    ...
    def action_view_wa(self):
        ...
        result["context"] = {
            ...
            "default_wa_line_ids": [
                ...
                if line._get_product_qty() != 0 and line.product_id.type == "service"
            ],
        }
    ...
    def action_create_invoice(self):
        for x in self:
            if x.purchase_order_type =='product':
                return super().action_create_invoice()
            enable_wa = x.env.user.has_group(
                "purchase_work_acceptance.group_enable_wa_on_invoice"
            )
            ctx = x.env.context.copy()
            ...
            if (
                ...
                wa = x.env["work.acceptance"].browse(ctx["wa_id"])
                invoice = x.env["account.move"].browse(res["res_id"])
    ...
    def action_sync_lines_qty_received(self):
        for order in self:
            for line in order.order_line:
                line.action_sync_qty_received()

    def action_download_planilla_summary(self):
        self.ensure_one()
        # return {
        #     'type': 'ir.actions.report',
        #     'report_type': 'xlsx',
        #     'report_name': 'purchase_work_acceptance.planillaje_report',
        #     'report_file': f'Resumen_Planilla_{self.name}',
        #     'data': {'purchase_id': self.id},
        # }
        return {
            'type': 'ir.actions.report',
            'report_type': 'xlsx',
...
class PurchaseOrderLine(models.Model):
    ...
    def _compute_qty_accepted(self):
        for line in self:
            qty_accepted = sum(
                wa_line.product_uom._compute_quantity(
                    wa_line.product_qty, line.product_uom, round=False
                )
                for wa_line in line.wa_line_ids.filtered(lambda l: l.wa_id.state == "accept")
            )
            line.qty_accepted = qty_accepted
            line.qty_to_accept = line.product_uom_qty - qty_accepted

    def action_sync_qty_received(self):
        for line in self:
            if line.product_id.detailed_type == 'service':
                line.qty_received = line.qty_accepted
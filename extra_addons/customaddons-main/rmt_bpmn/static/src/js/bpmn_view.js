...
import { Component, onMounted, useRef, useState, onWillDestroy } from  "@odoo/owl";
...
export class BPMNViewerAction extends Component {
    ...
    setup() {
        ...
        this.modelId = this.props.action.context.active_id == null ? parseInt(browser.localStorage.getItem('rmt_bpmn_model_id')) : this.props.action.context.active_id;
        this.diagramXML = defaultDiagramXML;
        this.state = useState({title: '', desc: '', dirty: false, modelId: this.modelId, isViewer: true});
    ...
    async onMounted() {
        ...
        this.record = await this.orm.read("rmt.bpmn.model", [this.modelId], []).then(ar => ar[0]);
...
export class BPMNModelerAction extends BPMNViewerAction {
    ...
    async onMounted() {
        ...
        this.modeler = new BpmnJS({
            ...
            additionalModules: [BpmnJSPropertiesPanel.BpmnPropertiesPanelModule, BpmnJSPropertiesPanel.BpmnPropertiesProviderModule],
            keyboard: {
                bindTo: window
            }
        });
        ...
        this.container.el.addEventListener('dragover', this.handleDragOverCallback, false);
        this.container.el.addEventListener('drop', this.handleFileSelectCallback, false);
        this.fileInput.el.addEventListener('change', this.handleFileSelectCallback)
    }

    onWillDestroy() {
        this.container.el.removeEventListener('dragover', this.handleDragOverCallback);
        this.container.el.removeEventListener('drop', this.handleFileSelectCallback);
        this.fileInput.el?.removeEventListener('change', this.handleFileSelectCallback)
    }
    ...
    async saveDiagram(ev) {
        try {
            ...
            await this.orm.write("rmt.bpmn.model", [this.modelId], { bpmn_data: encodeURIComponent(xml)});

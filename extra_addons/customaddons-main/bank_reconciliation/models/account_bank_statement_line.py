...
class AccountBankStatementLine(models.Model):
    ...
    matched_move_line_ids = fields.Many2many(
        "account.move.line",
        relation="bank_statement_line_matched_move_line",
        copy=False,
    )
    matched_manual_ids = fields.One2many(
        "account.bank.statement.manual", "statement_line_id"
    )
    matched_balance = fields.Monetary(
        compute="_calc_matched_balance", string="Open Balance"
    )
    matched_balance_absolute = fields.Monetary(
        compute="_calc_matched_balance", string="Open Balance (Absolute)"
    )

    reconcile_state = fields.Selection(
        [("Reconciled", "Reconciled"), ("Unreconciled", "Unreconciled")],
        compute="_calc_reconcile_state",
    ...
    statement_state = fields.Selection(
        related="statement_id.state", string="Statement Status"
    )
    ...
    def action_undo_reconciliation(self):
        res = super(AccountBankStatementLine, self).action_undo_reconciliation()
        for row in self:
            if row.matched_move_line_ids and row.matched_move_line_ids.reconciled:
                row.matched_move_line_ids.with_context(
                    skip_account_move_synchronization=True
                ).write({"reconciled": False, "statement_id": False})
        return res
    ...
    def _calc_edit_enabled(self):
        for record in self:
            record.edit_enabled = (
                not record.is_reconciled and record.statement_state != "confirm"
            )
                        ...
                        create_method(
                            {
                                ...
                                "tax_repartition_line_id": tax_res[
    ...
    def _calc_reconcile_state(self):
        for record in self:
            record.reconcile_state = (
                record.is_reconciled and "Reconciled" or "Unreconciled"
            )
    ...
    def button_reconciliation(self):
        ...
        return {
            ...
            "views": [
                (
                    ref(
                        "bank_reconciliation.view_bank_statement_line_form_reconciliation_popup"
                    ),
                    "form",
                )
            ],
    ...
    def _prepare_reconciliation(self, lines_vals_list, allow_partial=False):
        ...
        reconciliation_overview, open_balance_vals = super(
            AccountBankStatementLine, self
        )._prepare_reconciliation(lines_vals_list, allow_partial=allow_partial)
    ...
    @api.model
    def _group_by_account(self, moves):
        total_moves = defaultdict(lambda: self.env["account.move.line"])
        for move in moves:
            total_moves[move.account_id] |= move
        return total_moves.items()
    ...
    def action_reconcile(self):
        ...
        (
            ...
            suspense_lines,
            other_lines,
        ) = self._seek_for_lines()  # @UnusedVariable
        ...
        if (
            not self.matched_payment_ids.ids
            and not self.matched_move_line_ids.ids
            and not self.matched_manual_ids.ids
        ):
            raise ValidationError(
                _(
                    "Please fill out one of Payment Matching or Customer/Vendor Matching or Manual Operations"
                )
            )
        ...
        for account_id, lines in self._group_by_account(
            reconcile_lines + other_lines
        ):  # @UnusedVariable
            if account_id.reconcile and lines:
                lines.reconcile()
        ...
        for record in self:
            ...
            try:
                if record.is_reconciled:
                    ...
                    record.matched_balance = (
                        record.amount_residual
                        + sum(record._get_reconcile_lines().mapped("amount_residual"))
                        + sum(record.mapped("matched_manual_ids.balance"))
                    )
            except UserError:
                record.matched_balance = 0

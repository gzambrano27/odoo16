...
class AccountJournal(models.Model):
    ...
    reconciliation_seq = fields.Many2one(
        comodel_name="ir.sequence", string=_("Bank Statement Sequence")
    )

    def get_journal_dashboard_datas(self):
        res = super(AccountJournal, self).get_journal_dashboard_datas()
        isEnterprise = (
            odoo.service.common.exp_version()["server_version_info"][-1] == "e"
        )
        res.update(
    ...
    def action_statement_reconcile(self):
        ...
        return {
            ...
            "views": [
                (
                    ref(
                        "oi_bank_reconciliation.view_bank_statement_line_tree_reconciliation"
                    ),
                    "tree",
                ),
                (
                    ref(
                        "oi_bank_reconciliation.view_bank_statement_line_form_reconciliation"
                    ),
                    "form",
                ),
            ],

...
class PurchaseRequestAllocation(models.Model):
    ...
    def _compute_open_product_qty(self):
        for rec in self:
            ...
            if rec.purchase_request_line_id.employees_ids:
                #coger una lista los ids de employees_ids de la purchase_request_line_id
                values = rec.purchase_request_line_id.employees_ids.mapped("id")
                #rec.stock_move_id.move_line_ids.write({"employee_id": values.id[0]})

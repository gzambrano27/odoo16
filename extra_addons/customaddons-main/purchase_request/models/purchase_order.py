...
class PurchaseOrder(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if company.require_requisition_po:
                if not vals.get('purchase_request_id') and not vals.get('custom_requisition_id'):
                    raise UserError(_("No puedes crear una orden de compra sin estar vinculada a una requisición de materiales o de servicios."))
...
class PurchaseOrderLine(models.Model):
    ...
    def onchange_product_qty(self):
        # if (self.product_qty <= 0) or (self.product_qty is None):
        #     raise UserError(_("La cantidad del producto '%s' no puede ir con valor 0, ni negativo.", self.product_id.default_code))
        self.product_brand_id = self.product_id.product_tmpl_id.product_brand_id.id
        if self.order_id.purchase_request_id:
            if self.product_qty > (self.purchase_request_allocation_ids.requested_product_uom_qty):
                self.product_qty=self.purchase_request_allocation_ids.requested_product_uom_qty
                return {'warning': {
                    'title': _("Cantidad inválida"),
                    'message': _("La cantidad del producto '%s' no puede ser mayor a lo solicitado en la orden generada.") % (

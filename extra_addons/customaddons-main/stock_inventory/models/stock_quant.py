...
class StockQuant(models.Model):
    ...
    def action_apply_inventory(self):
        ...
        if inventory:
            ...
            if inventory.name in ("/", _("New")):
                sequence = self.env.ref("stock_inventory.sequence_inventory_doc")
                if sequence:
    ...
    def write(self, vals):
        if "inventory_quantity" in vals and not self.env.user.has_group(
            "stock_inventory.group_view_inventory_button"
        ):
            raise UserError(_("Your user cannot update product quantities"))
        res = super().write(vals)
        if "inventory_quantity" in vals and not self.env.context.get(
            ...
            for quant in self:
                ...
                if (
                    inventor_line
                    and inventor_line.product_qty != quant.inventory_quantity
                ):
                    inventor_line.write(
                        {"product_qty": quant.inventory_quantity, "is_ok": True}
                    )
                # else:

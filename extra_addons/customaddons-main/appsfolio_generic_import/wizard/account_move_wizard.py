...
class AccountMoveWizard(models.TransientModel):
    ...
    def check_salesperson(self, name):
        sals_person_obj = self.env['res.users']
        partner_search = sals_person_obj.search([
            ('name', '=', name)
        ])
        if partner_search:
            return partner_search[0]
            ...
            raise ValidationError(
                _('Not Valid Salesperson Name "%s"') % name)
    ...
    def check_currency(self, name):
        ...
        if currency_search:
            ...
            raise ValidationError(
                _(' "%s" Currency are not available.') % name)
    ...
    def create_invoice(self, values):
        ...
        if invoice_search:
            if invoice_search.partner_id.name == values.get('customer'):
                if invoice_search.currency_id.name == values.get('currency'):
                    if invoice_search.user_id.name == values.get('salesperson'):
                        self.create_invoice_line(values, invoice_search)
            ...
            salesperson_id = self.check_salesperson(values.get('salesperson'))
            inv_date = self.check_invoice_date(values.get('date'))
            ...
            if self.type == "in":
                ...
                if partner_id.property_account_receivable_id:
                    ...
                    if not account_id:
                        raise UserError(_('Define Customer account.'))
                ...
                if partner_id.property_account_payable_id:
                    ...
                    account_id = account_search.value_reference
                    if not account_id:
                        raise UserError(_('Define Vendor account.'))
                    account_id = account_id.split(",")[1]
                    account_id = self.env['account.account'].browse(account_id)
                ...
                if partner_id.property_account_receivable_id:
                    ...
                    if not account_id:
                        raise UserError(
                            _('Please define Customer account.'))
                    account_id = account_id.split(",")[1]
                    account_id = self.env['account.account'].browse(account_id)
            else:
                type_inv = "in_refund"
                if partner_id.property_account_payable_id:
                    ...
                    if not account_id:
                        raise UserError(
                            _('Please define Vendor account.'))
                    account_id = account_id.split(",")[1]
                    account_id = self.env['account.account'].browse(account_id)
                                ...
                                if many2x_fields.ttype == "many2one":
                                    if values.get(i):
                                        ...
                                        if fetch_m2o.id:
                                            ...
                                            raise ValidationError(
                                                _('"%s" this custom field value "%s" not available') % (many2x_fields.name, values.get(i)))
                                    ...
                                    if values.get(i):
                                        if ';' in values.get(i):
                                            ...
                                            for name in m2m_names:
                                                ...
                                                if not m2m_id:
                                                    raise ValidationError(
                                                        _('"%s" this custom field value "%s" not available') % (many2x_fields.name, name))
                                            ...
                                            if not m2m_id:
                                                raise ValidationError(
                                                    _('"%s" this custom field value "%s" not available') % (many2x_fields.name, m2m_names))
                                            m2m_value_lst.append(m2m_id.id)
                                    inv_id.update({
            ...
            self.create_invoice_line(values, inv_id)
            if values.get('seq_opt') == 'system':
                inv_id.update({'name': '/'})
            return inv_id

    def create_invoice_line(self, values, inv_id):
        ...
        if product_search:
            ...
            if self.option_import_file_by == 'name':
                if values.get('price'):
        ...
        if self.account_option == 'default':
            ...
            if values.get('account') == '':
                raise ValidationError(
                    _("Please do not leave the 'Account' field blank if you choose the 'Excel/CSV Account' option."))
            else:
                if self.file_import_type == 'csv':
    ...
    def check_invoice_date(self, date):

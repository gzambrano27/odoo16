						...
						if self.product_option == 'create':
							...
							if values.get('categ_id') == '':
								...
								if not categ_id:
									raise ValidationError(
										_('Category %s not found.' %values.get('categ_id') ))
							if values.get('type') == '':
								pass
							else:
							...
							if values.get('uom_id') == '':
								...
								uom_search_id = product_uom_obj.search([
									('name', '=', values.get('uom_id'))
								])
								if not uom_search_id:
									raise ValidationError(
										_('UOM %s not found.' %values.get('uom_id')))
								else:
							...
							if values.get('uom_po_id') == '':
								...
								uom_po_search_id = product_uom_obj.search([
									('name', '=', values.get('uom_po_id'))
								])
								if not uom_po_search_id:
									raise ValidationError(
										_('Purchase UOM %s not found' %values.get('uom_po_id')))
								else:
							...
							if values.get('taxes_id'):
								if ';' in values.get('taxes_id'):
									...
									for name in tax_names:
										...
										if not tax:
											raise ValidationError(
												_('"%s" tax not in your system') % name)
									...
									if not tax:
										raise ValidationError(
											_('"%s" tax not in your system') % tax_names)
									tax_id_lst.append(tax.id)
							...
							if self.product_search == 'by_code':
								...
								if not values.get('default_code'):
									raise ValidationError(
										_('Please give Internal Reference for updating Products'))
								...
								if product_ids:
									if categ_id != False:
										product_ids.write({
											'categ_id': categ_id[0].id or False
										})
									if categ_type != False:
										product_ids.write({
									...
									if values.get('sale_price'):
										product_ids.write({
											'list_price': values.get('sale_price') or False
										})
									...
									raise ValidationError(
										_('"%s" Product not found.') % values.get('default_code'))
							
							elif self.product_search == 'by_name':
								if not values.get('name'):
									raise ValidationError(
										_('Please give Name for updating Products'))
								...
								if not barcode:
									raise ValidationError(
										_('Please give Barcode for updating Products'))
								...
								if product_ids:
																...
																if values.get(i):
																	if ';' in values.get(i):
																		...
																		for name in m2m_names:
																			...
																			if not m2m_id:
																				raise ValidationError(
																					_('"%s" this custom field value "%s" not available in system') % (i , name))
																			m2m_value_lst.append(m2m_id.id)
									...
									raise ValidationError(
										_('%s product not found.') % values.get('barcode'))
			return res

		if self.file_import_type == 'xls':
			try:
				...
				raise ValidationError(
					_("Please give an Excel File for Importing Products!"))

			for row_no in range(sheet.nrows):
				val = {}
				if row_no <= 0:
					line_fields = list(map(
					...
					if self.product_option == 'create':
						...
						if line[6]=='':
							...
							if not uom_po_search_id:
								raise ValidationError(
									_('Purchase UOM %s not found' %line[6]))
							else:
								uom_po_id = uom_po_search_id.id
						...
						if self.product_search == 'by_code':
							...
							if product_ids:
								...
								if line[0]:
									product_ids.write({
										'name': line[0] or False
									})
								if line[7]:
									product_ids.write({
										'list_price': line[7] or False
									})
								if line[8]:
									product_ids.write({
										'standard_price': line[8] or False
									})
								if line[9]:
									product_ids.write({
										'weight': line[9] or False
									})
								if line[10]:
									product_ids.write({
										'volume': line[10] or False
									})
														...
														if many2x_fields.ttype =="many2one":
															if line[count]:
																...
																if fetch_m2o.id:
																	...
																	raise ValidationError(
																		_('"%s" this custom field value "%s" not available') % (technical_fields_name , line[count]))
														...
														if many2x_fields.ttype =="many2many":
															...
															if line[count]:
																if ';' in line[count]:
																	...
																	if not m2m_id:
																		raise ValidationError(
																			_('"%s" this custom field value "%s" not available') % (technical_fields_name , m2m_names))
																	m2m_value_lst.append(m2m_id.id)
															product_ids.update({
																technical_fields_name: m2m_value_lst
															})
			...
			if values.get('attributes'):
				...
				for pair in atr_value:
					...
					for att in attr_values:
						if att == '':
							raise ValidationError(_('Please give values after ;'))

...
class SaleOrderWizard(models.TransientModel):
            ...
            for i in range(len(file_reader)):
                ...
                if values:
                    if i == 0:
                        ...
                        res = self.create_sale_order(values)
                        sale_ids.append(res)
    ...
    def check_user(self, name):
        ...
        if user_search:
            ...
            raise ValidationError(
                _(' "%s" User is not available.') % name)
        ...
        if sale_search:
            ...
            if sale_search.partner_id.name == values.get('customer'):
                if sale_search.pricelist_id.name == values.get('pricelist'):
                    lines = self.create_sale_order_line(values, sale_search)
                    return sale_search
                else:
                    raise ValidationError(
            ...
            currency_id = self.check_currency(values.get('pricelist'))
            user_id = self.check_user(values.get('user'))
            order_date = self.check_sale_order_date(values.get('date'))
            ...
            lines = self.create_sale_order_line(values, sale_id)
            return sale_id
    ...
    def check_currency(self, name):
        ...
        if currency_search:
            ...
            raise ValidationError(
                _(' "%s" Pricelist are not available.') % name)

    def check_sale_order_date(self, date):
        ...
        if date:
            ...
            raise ValidationError(
                _('Please add the date field in sheet.'))

    def create_sale_order_line(self, values, sale_id):
        ...
        if self.option_import_file_by == 'barcode':
            ...
            if prefix:
                ...
                if res == True:
                    product_search = product_obj.search([
                        ('barcode', '=',
                         str(values['product']).rstrip('0').rstrip('.') if '.' in str(values['product']) else str(
                             values['product']))
                    ])
                else:
                    product_search = product_obj.search([

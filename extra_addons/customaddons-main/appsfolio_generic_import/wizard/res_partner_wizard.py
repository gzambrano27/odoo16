...
class ResPartnerWizard(models.TransientModel):
                        ...
                        if self.partner_option == 'create':
                            ...
                            if values.get('type').lower() == 'company':
                                if values.get('parent'):
                                    raise ValidationError(
                                        _('If you have selected the Company type, you cannot specify a parent.'))
                                type = 'company'
                            else:
                                type =  'person'
                                ...
                                if values.get('parent'):
                                    ...
                                    if parent_search:
                                        ...
                                        raise ValidationError(
                                            _("Parent contact is not available"))
                            
                            if values.get('state'):
                                state = self.check_state(values)
                            if values.get('country'):
                                country = self.check_country(values)
                            if values.get('saleperson'):
                                ...
                                if not saleperson_search:
                                    raise ValidationError(
                                        _("Salesperson is not available"))
                                else:
                            ...
                            if values.get('cust_pmt_term'):
                                ...
                                if not cust_payment_term_search:
                                    raise ValidationError(
                                        _("Payment term not available in system"))
                                else:
                ...
                if row_no <= 0:
                    ...
                    if self.partner_option == 'create':
                        ...
                        if line[1] == 'company':
                            ...
                            if line[2]:
                                ...
                                if parent_search:
                                    ...
                                    raise ValidationError(
                                        _("Parent contact not available"))
                        
                        if line[6]:
                            state = self.check_state(line)
                        if line[9]:
                            country = self.check_country(line)
                        if line[16]:
                            saleperson_search = self.env['res.users'].search([
                                ('name', '=', line[16])
                            ])
                        ...
                        if search_partner:
                            ...
                            raise ValidationError(
                                _('%s partner not found.') % line[0])
        
        return res
    
    def check_country(self,val):
        if type(val) == dict:
        ...
        if type(val) == dict:
            ...
            if state_search:
                if len(state_search.ids)> 1:
                    raise ValidationError(
                        _('Multiple States of name %s found. Please provide Country.'%val.get('state')))
                else:
                    return state_search.id
            else:
                if not val.get('country'):
                    raise ValidationError(
                        _('States are not available in the system, and you cannot create a state without specifying the country.'))
                else:
                    country_search = self.env['res.country'].search([
                        ('name', '=', val.get('country'))
                    ])
                    ...
                    if states:
                        if not val.get('state_code'):
                            raise ValidationError(
                                _('Another State with code "%s" found in country "%s". Please Provide State Code in XLS/CSV.'%(val.get('state')[:3],country.name) ))
                        else:
            ...
            if state_search:
                if len(state_search.ids)> 1:
                    raise ValidationError(
                        _('Multiple States of name %s found. Please provide Country.'%val[6]))
                else:
                    return state_search.id
            else:
                if not val[9]:
                    ...
                    if states:
                        if not val[7]:
                            raise ValidationError(
                                _('Another State with code "%s" found in country "%s". Please Provide State Code in XLS/CSV.'%(val[6][:3],country.name) ))
                        else:
                            code = str(val[7])
                    else:
                        code = val[6][:3]
    ...
    def create_partner(self, values):
        ...
        if values.get('type').lower() == 'company':
            ...
            if values.get('parent'):
                ...
                if parent_search:
                    ...
                    raise ValidationError(_("Parent contact is not available!"))
        ...
        if values.get('saleperson'):
            ...
            if not saleperson_search:
                raise ValidationError(_("Salesperson not available."))
            else:
                saleperson = saleperson_search.id
        ...
        if partner_search:
            raise ValidationError(
                _('"%s" Customer/Vendor already exist.') % values.get('name'))
        else:

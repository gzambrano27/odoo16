...
class PurchaseOrderWizard(models.TransientModel):
    ...
    def import_purchase_order_file(self):
        if self.file_import_type == 'csv':
            try:
                keys = ['purchase_no', 'vendor', 'currency', 'product', 'quantity', 'uom', 'description', 'price',
                        'tax', 'date']
            ...
            for i in range(len(file_reader)):
                ...
                if values:
                    if i == 0:
                        ...
                        res = self.create_purchase_order(values)
    ...
    def check_currency(self, name):
        currency_search = self.env['res.currency'].search([
            ('name', '=', name)
        ])
        if currency_search:
            ...
            raise ValidationError(
                _(' "%s" Currency not available.') % name)
    ...
    def create_purchase_order(self, values):
        ...
        if pur_search:
            if pur_search.partner_id.name == values.get('vendor'):
                if pur_search.currency_id.name == values.get('currency'):
                    self.create_purchase_order_line(values, pur_search)
                    return pur_search
                else:
                    raise ValidationError(
            ...
            if values.get('date'):
                pur_date = self.create_purchase_order_date(values.get('date'))
            else:
        ...
        self.create_purchase_order_line(values, pur_id)
        return pur_id

    def create_purchase_order_date(self, date):
    ...
    def create_purchase_order_line(self, values, pur_id):

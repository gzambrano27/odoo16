...
class MrpBomWizard(models.TransientModel):
            ...
            for i in range(len(file_reader)):
                ...
                if values:
                    if i == 0:
                        ...
                        res = self.create_mrp_bom(values)
        else:
            try:
    ...
    def create_mrp_bom(self, values):
        ...
        if self.option_import_file_by == 'barcode':
            product_obj_search=self.env['product.template'].search([
                ('barcode', '=', values.get('product_tmpl').split('.')[0])
            ], limit=1)
        elif self.option_import_file_by == 'code':
            product_obj_search=self.env['product.template'].search([
                ('default_code', '=', values.get('product_tmpl'))
            ], limit=1)
        else:
            product_obj_search=self.env['product.template'].search([
                ('name', '=', values.get('product_tmpl'))
            ], limit=1)
        ...
        if product_obj_search:
            ...
            raise ValidationError(
                _('%s product is not found.') % values.get('product_tmpl').split('.')[0])
            
        if bom_search:
            if  bom_search[0].code == values.get('ref'):
                if bom_search[0].product_tmpl_id.name != product_tmpl_id.name:
                   raise ValidationError(
                    _('Found Diffrent value of product for same BOM %s') % product_tmpl_id)
                else:    
                    ...
                    self.create_mrp_bom_line(values, bom_search[0])
                return bom_search
            else:
                raise ValidationError(
                    _('Found Diffrent value same BOM %s') % values.get('ref'))
        else:
                ...
                if normal_details.startswith('x_'):
                    ...
                    if any_special:
                        ...
                        if normal_fields.id:
                            ...
                            raise ValidationError(
                                _('"%s" this custom field is not available') % normal_details)
            self.create_mrp_bom_line(values, bom_id)
            return bom_id
    
    def create_mrp_bom_line(self, values, bom_id):
        product_id = False
        product_obj = self.env['product.product']
        mrp_line_obj = self.env['mrp.bom.line']
        product_uom = self.env['uom.category'].search([
            ('name', '=', values.get('uom'))
        ])
        ...
        if product_obj_search:
            ...
            raise ValidationError(
                _('%s product is not found.') % values.get('product'))
        if not product_uom:
            raise ValidationError(

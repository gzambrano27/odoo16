...
class ImportInvoiceWizard(models.TransientModel):
    ...
    def import_invoice_line_file(self):
            ...
            for row_no in range(sheet.nrows):
                ...
                if row_no <= 0:
                    ...
                    for l_fields in range(0, len(line)):
                        if (count > 6):
                            values.update({
                                line_fields[l_fields]: line[count]
                            })
                        count += 1
                    res = self.create_inv_line(values)
        return res
    ...
    def create_inv_line(self, values):
        ...
        if self.product_details_option == 'from_product':
            ...
            if account_inv_brw.move_type == "out_invoice" and account_inv_brw.state == 'draft':
                ...
                raise UserError(
                    _('Can not import data in validated or confirmed Invoice.'))
            ...
            uom_obj_search = self.env['uom.uom'].search([
                ('name', '=', uom)
            ])

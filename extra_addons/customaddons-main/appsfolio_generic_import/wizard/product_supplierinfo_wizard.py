...
class ProductSupplierinfoWizard(models.TransientModel):
    _name = "product.supplierinfo.wizard"
    _description = "Product Supplierinfo Wizard"
    ...
    ], string='Product Option', default='link')
    ...
    def import_product_supplier_file(self):
        try:
        ...
        for row_no in range(sheet.nrows):
            if row_no <= 0:
                ...
                for l_fields in line_fields:
                    ...
                    count += 1
                res = self.create_product_supplierinfo(values)
        return res

    def check_product_template(self,product,create_opt):
        ...
        if not product_tmpl_search:
            if create_opt == 'create':
                ...
                raise ValidationError (
                    _("You have selected Link product template with existing product but %s Product template does not exist") % product)
        return product_tmpl_search.id

    def check_vendor(self,name):
    ...
    def create_product_supplierinfo(self,values):
        name = self.check_vendor(values.get('vendor'))
        product_tmpl_id = self.check_product_template(
            values.get('product'),
            values.get('create_link_option')
        )

...
class LabRequest(models.Model):
    ...
    _description = 'Solicitud de Consultorio'
    ...
    state = fields.Selection([
        ('draft', 'Borrador'),
        ('sample_collection', 'Muestra Recolectada'),
        ('test_in_progress', 'Test In Progress'),
        ('completed', 'Completada'),
        ('cancel', 'Cancelada'),

    ], string='Estado', readonly=True, copy=False, index=True, tracking=True, default='draft')
    ...
    def set_to_test_completed(self):
        if not self.request_line:
            raise ValidationError(_("No se ingresaron lineas de resultados !"))

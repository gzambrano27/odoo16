...
class DocumentFinancialLineReportHandler(models.AbstractModel):
    ...
    def _report_custom_engine_aged_c(self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None):
        total_documentos= self._report_custom_engine_cobros_aged(None, options, date_scope, current_groupby, next_groupby, offset=offset, limit=limit)
        total_cxc=self._report_custom_engine_cxc_aged(None,options, 'total', current_groupby, next_groupby, offset=offset, limit=limit)
        campos_a_sumar = ['period0', 'period1', 'period2', 'period3', 'period4', 'period5', 'total']

        for campo in campos_a_sumar:
            valor_doc = total_documentos.get(campo) or 0
            valor_cxc = total_cxc.get(campo) or 0
            total_documentos[campo] = valor_doc + valor_cxc
        return total_documentos


    def _report_custom_engine_aged_full(self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None):
        total_documentos= self._report_custom_engine_aged_c(expressions, options, date_scope, current_groupby, next_groupby, offset=offset, limit=limit)
        total_cxp=self._report_custom_engine_aged(expressions, options, date_scope, current_groupby, next_groupby, offset=offset, limit=limit)
        campos_a_sumar = ['period0', 'period1', 'period2', 'period3', 'period4', 'period5', 'total']

        for campo in campos_a_sumar:
            valor_cxc = total_documentos.get(campo) or 0
            valor_cxp = total_cxp.get(campo) or 0
            total_documentos[campo] = valor_cxc - valor_cxp
        return total_documentos

...
class StockQuant(models.Model):
	...
	def create(self, vals):
		...
		if self._is_inventory_mode() and any(f in vals for f in ['inventory_quantity', 'inventory_quantity_auto_apply']):
			...
			lot_id = vals.get('lot_id') and self.env['stock.production.lot'].browse(vals['lot_id']) or False
...
class StockMove(models.Model):
	...
	def _action_done(self, cancel_backorder=False):
		...
		force_date = datetime.now().strftime(DEFAULT_SERVER_DATETIME_FORMAT)
		if self.env.user.has_group('stock_force_date_app.group_stock_force_date'):
			for move in self:
		...
		self._cr.execute('''
			...
			INNER JOIN account_move move2 ON
				...
				AND move2.id != move.id
				   AND move.partner_id = move2.partner_id

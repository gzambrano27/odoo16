...
class TradeImportation(models.Model):
    ...
    state = fields.Selection([
        ...
        ('distribution', 'DISTRIBUCIÓN DE ARANCELES'),
    ...
    partner_ids = fields.Many2many('res.partner', string="Proveedores", required=True, store=True)
    
    provider_names = fields.Char(string='Proveedor(es)', compute='_compute_provider_names', store=True)

    # purchase_ids = fields.One2many('purchase.order', 'importation_id', string='Ordenes de Compra',
    #                                domain="[('state','=', 'purchase'), ('partner_id', '=', partner_id), ('state_confirmed', '=', False)]")
    purchase_ids = fields.One2many('purchase.order', 'importation_id', string='Ordenes de Compra', store=True,
                                   domain="[('state','=', 'purchase'), ('partner_id', 'in', partner_ids)]")
    ...
    def _compute_provider_names(self):
        for rec in self:
            rec.provider_names = ', '.join(rec.partner_ids.mapped('name')) if rec.partner_ids else ''
    ...
    porcentaje_isd = fields.Float(string='Porcentaje ISD', digits=dp.get_precision('Import calculations'),  store=True)
    total_isd = fields.Float(string='Total ISD', digits=dp.get_precision('Import calculations'), compute='_compute_isd', store=True)

    @api.depends('liq_fob', 'porcentaje_isd', )
    def _compute_isd(self):
        for record in self:
            if record.liq_fob and record.porcentaje_isd:
                record.total_isd =  (record.liq_fob or 0.0) * record.porcentaje_isd/100
            else:
                record.total_isd = 0.0
    ...
    def confirmed_importation(self):
        for order in self:
            if not order.importation_rnote_ids:
                self.update_purchase_ids()
                #raise exceptions.UserError(_('No hay lineas de pedido para realizar la importación'))
            for line in order.importation_rnote_ids:
                if line.price == 0:
                    raise exceptions.UserError(_('You must enter the price of the Foreign'))
                elif line.product_qty == 0:
                    raise exceptions.UserError(_('Quantity must be non-zero'))

            list = []
            self.aranceles_line_ids.unlink()
            for line in self.importation_rnote_ids:
                imp_line_data = {'importation_line': line.id,
                                 'importation_id': self.id,
                                 'qty': line.product_qty,
                                 'subtotal_real': float(line.price_subtotal),
                                 'def_fob_item': float(line.price_subtotal),
                                 'def_fob_recibido': float(line.price_subtotal),
                                 }
                list.append((0, 0, imp_line_data))
            self.aranceles_line_ids = list

            order.write({'state': 'confirmed'})

    # @api.onchange('partner_ids')
    # def _onchange_partner_ids_load_po(self):
    #     if self.partner_ids:
    #         domain = [
    #             ('state', '=', 'purchase'),
    #             ('partner_id', 'in', self.partner_ids.ids),
    #             ('state_confirmed', '=', False),
    #             ('importation_id', '=', False)  # solo las que aún no están asignadas
    #         ]
    #         orders = self.env['purchase.order'].search(domain)
    #         self.purchase_ids = [(4, po.id) for po in orders]
    #     else:
    ...
    def generar_distribucion_aranceles(self):
        ...
        if not self.invoices_ids and not self.voucher_ids:
            raise exceptions.UserError("Debes cargar los documentos de facturas o recibos, en Documentos Electrónicos!!")

        # landed_cost = self.env["ir.config_parameter"].sudo().get_param( "stock_account.module_stock_landed_costs")
        # if not landed_cost:
        #     raise exceptions.UserError(
        #         "Por favor habilite la opción Costos en Destino en Ajustes/Inventario")

        if self.picking_ids:
            if self.picking_ids.filtered(lambda x: x.state not in ('done')):
                msg = 'Por favor valide la recepción de la mercadería para continuar con el proceso.'
                raise exceptions.UserError(msg)

        for record in self.aranceles_line_ids:
            if record.product_id.arancel_ids:
                save = 0
                adv = 0
                for item in record.product_id.arancel_ids:
                    if item.type_arancel == 'svg' or item.type_arancel == 'svgesp':
                        save += item.percent
                    if item.type_arancel == 'adv' or item.type_arancel == 'advspe':
                        adv += item.percent
            else:
                save = False
                adv = False
            record.def_adval_percents = adv
            record.def_salva_percents = save
    ...
    def confirm_foreign(self):
        for rec in self:
            if rec.picking_ids:
                if rec.picking_ids.filtered(lambda x: x.state not in ('done')):
                    msg = 'Por favor valide la recepción de la mercadería para continuar con el proceso.'
                    raise exceptions.UserError(msg)
            if not self.clase_importacion:
                raise exceptions.UserError("Ingrese una clase de importación")
            self.cambiar_cuenta_picking()
            self._generar_costos_destino()
            self.obtener_costos_productos()

            for order in self.purchase_ids:
                for line in order.order_line:
                    if line.qty_received == line.product_qty:
                        order.write({'state_confirmed': True})
    ...
    def _generar_costos_destino(self):
        ...
        if landed.account_move_id:
            landed.account_move_id.date = self.liq_date
            for line in landed.account_move_id.line_ids:
                line.date = self.liq_date
    ...
    def cambiar_cuenta_picking(self):
        for purchase in self:
            ...
            #moves.write({'state': 'draft','date':fields.Date.today()})
            #moves.write({'state': 'draft'})
            for line in moves.mapped('line_ids'):
                if line.move_id.state == 'posted':
                    # Ya realizado, no tocar
                    continue
            ...
            #moves.action_post()
                line.with_context(force_account_date=original_date).action_post()
                line.date = original_date
                for x in line.line_ids:
                    x.date = original_date
...
class ImportationTradeLine(models.Model):
    ...
    @api.depends('price', 'product_qty','descuento')
    def _compute_fob_item(self):
        for record in self:
            precio_descuento = record.price * (1 - (record.descuento or 0.0) / 100)
            record.price_subtotal = precio_descuento * record.product_qty
    ...
    advalorem = fields.Float(string='Advalorem')
    fodinfa = fields.Float(string='Fodinfa')
    product_uom = fields.Many2one('uom.uom', string='Unit of Measure', domain="[('category_id', '=', product_uom_category_id)]")
    product_uom_category_id = fields.Many2one(related='product_id.uom_id.category_id')
    fob_unit_price = fields.Float(string='FOB Unit Price')
    fob_total = fields.Float(string='FOB Total')
    descuento = fields.Float(string='% Dscto')

    @api.depends('importation_id.aranceles_line_ids')
    def _obtener_advalorem(self):
        for rec in self:
            # Filtrar los productos relacionados
            prod_id = rec.importation_id.aranceles_line_ids.filtered(lambda x: x.product_id == rec.product_id)
            if prod_id:
                # Verificar si 'def_adval_percents' existe y tiene valor
                rec.def_adval_percents = prod_id[0].def_adval_percents if prod_id[0].def_adval_percents else 0.0
            else:
                # Asignar valor predeterminado si no hay coincidencia
                rec.def_adval_percents = 0.0     
        # for rec in self:
        #     prod_id = rec.importation_id.aranceles_line_ids.filtered(lambda x: x.product_id == rec.product_id)
        #     if prod_id.def_adval_percents:
        #         rec.def_adval_percents = prod_id.def_adval_percents

    def_adval_percents = fields.Float('Advalorem', compute='_obtener_advalorem')

    # @api.onchange('product_id')
    # def onchange_product_id_tarrif(self):
    #     warning = {}
    #
    #     product_info = self.product_id
    #     title = _("Notice for %s") % product_info.name
    #     warning['title'] = title
    #     if not product_info.from_trade:
    #         message = ('Product not configured for import')
    #         warning['message'] = message
    #         self.product_id = False
    #         return {'warning': warning}
    #     elif not product_info.purchase_tariff_id and not product_info.purchase_tariff_id.arancel_ids:
    #         message = ('You must assign tariff to an import product')
    #         warning['message'] = message
    #         return {'warning': warning}
    #     return {}
...
class PurchaseInvoice(models.Model):
    ...
    amount = fields.Monetary(string='Total with VAT', store=True, )
...
class PurchaseAranceles(models.Model):
    ...
    @api.depends('def_insurance', 'def_fob_item', 'def_fob_recibido','def_freight')
    def _compute_cif_distribution(self):
        for record in self:
            record.def_cif = float(record.def_insurance + record.def_fob_recibido + record.def_freight)

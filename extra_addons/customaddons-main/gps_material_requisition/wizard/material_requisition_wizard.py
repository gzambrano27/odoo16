...
class MaterialRequisitionWizard(models.TransientModel):
    ...
    def get_default_company_id(self):
        if self._context.get("active_ids",False):
            for brw_each in self.env["material.purchase.requisition"].sudo().browse(self._context["active_ids"]):
                return brw_each.company_id.id
        return False


    movement_id=fields.Many2one("material.purchase.requisition", "Requisicion",required=False,default=get_default_movement_id)
    ...
    def process_file(self):
        ...
        OBJ_LINE = self.env["material.purchase.requisition.line"].sudo()
        ACTIVE_LINE_ID, Producto, Descripcion, Rubro, CtaAnalitica, Unidad, Cantidad, Precio, Pvp = 0, 1, 2, 3, 4, 5, 6, 7, 8
        for brw_each in self:
            line_ids = []
            ...
            for row_index, row in enumerate(sheet.iter_rows(min_row=4, values_only=True), start=4):  # Comienza desde la fila 4
                ...
                values = {
                    ...
                    'analytic_account_id': srch_cta[0].id if ctaanalitica else ''
                }
